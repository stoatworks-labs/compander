...
namespace
{
constexpr const char* kAppName = "compander";
...
{
	const std::string override_ = environmentVariable( "COMPANDER_LOG_DIR" );
	if( !override_.empty() )
		return override_;
...
{
	error( std::string( "shader stage '" ) + stage + "' failed to compile" );

	// The ASSEMBLED source, numbered. Three of these shaders are spliced
	// together at runtime, so a line number in the compiler's message refers to
	// this and to no file on disk.
	if( source == nullptr )
		return;

	info( "  assembled source follows" );
	...
	for( const char* p = source; *p != '\0'; ++p )
	{
		if( *p == '\n' )
		{
			info( "  " + std::to_string( number++ ) + ": " + line );
			line.clear();
		}
		else
		{
...
{
	error( "buffer allocation failed at " + std::to_string( width ) + "x" + std::to_string( height ) +
	       " -- five full-resolution GL_R16F buffers plus two at quarter width" );
}
